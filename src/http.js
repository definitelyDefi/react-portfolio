...
import { setSearchFetch, setAnimes, setGenres } from './store/reducers/animeReducer'
import { setViewFetch, setCurrAnime } from './store/reducers/animeReducer'
import { searchWeather, setWeatherError, setWeatherFetch } from './store/reducers/weatherReducer'
import { reset_anime } from './store/reducers/animeReducer'


export const weatherApiCallThunk = (query_city, API_KEY) => async dispatch =>{
    dispatch(setWeatherFetch())
    await fetch(`https://api.weatherbit.io/v2.0/forecast/daily?city=${query_city}&key=${API_KEY}`)
.then(res => res.json()).then(result => {dispatch(searchWeather({weather: result.data, country: result.country_code}) )}).catch(()=> dispatch(setWeatherError()))
}
...
export const fetchAnimeThunk = animeName => async dispatch => {
    ...
    .then(res => res.json()).then(result => dispatch(setAnimes(result.data)))
}
...
export const fetchGenresThunk = animeid => async dispatch => {
    ...
    .then(res => res.json()).then(result => dispatch(setGenres(result.data)))
}